...
SoE_BLISS <- function(species, gpkg_path, niterations,nchains, burnin, nthin){
  ...
  x.range<- read_sf(gpkg_path, layer= "range")
  ...
  vars<- c("open_water","snow","developed_open_space","developed_low_intensity","developed_medium_intensity","developed_high_intensity",
  ...
  Nimble_BLISS<- nimbleCode( {
    ...
    scale.1 ~ dcat (priors[1:50]) 
    scale.2 ~ dcat (priors[1:50]) 
    scale.3 ~ dcat (priors[1:50]) 
    scale.4 ~ dcat (priors[1:50]) 
    scale.5 ~ dcat (priors[1:50]) 
    scale.6 ~ dcat (priors[1:50]) 
    scale.7 ~ dcat (priors[1:50]) 
    scale.8 ~ dcat (priors[1:50]) 
    scale.9 ~ dcat (priors[1:50]) 
    scale.10 ~ dcat (priors[1:50]) 
    scale.11 ~ dcat (priors[1:50]) 
    scale.12 ~ dcat (priors[1:50]) 
    scale.13 ~ dcat (priors[1:50]) 
    scale.14 ~ dcat (priors[1:50]) 
    scale.15 ~ dcat (priors[1:50]) 
    scale.16 ~ dcat (priors[1:50]) 
    scale.17 ~ dcat (priors[1:50]) 
    scale.18 ~ dcat (priors[1:50]) 
    scale.19 ~ dcat (priors[1:50]) 
    scale.20 ~ dcat (priors[1:50]) 
    scale.21 ~ dcat (priors[1:50])
    ...
    for (i in 1:nlocalities){
      logit_psi[i] <- beta.0 + beta.developed_low_intensity * developed_low_intensity [i, scale.1] + beta.developed_high_intensity * developed_high_intensity [i, scale.2]+ 
      beta.barren_land * barren_land [i, scale.3] + beta.deciduous_forest * deciduous_forest [i,scale.4] + beta.evergreen_forest * evergreen_forest [i, scale.5] + beta.mixed_forest * mixed_forest [i, scale.6]+ 
      beta.shrub_scrub * shrub_scrub [i, scale.7] + beta.grassland_herbaceous * grassland_herbaceous [i, scale.8] + beta.pasture_hay * pasture_hay [i, scale.9] + beta.cultivated_crop * cultivated_crop [i, scale.10] + beta.woody_wetland * woody_wetland [i,scale.11] + beta.emergent_wetland * emergent_wetland [i, scale.12]+
      beta.ocean * ocean [i, scale.13] + beta.lakes * lakes [i, scale.14] + beta.rivers * rivers [i, scale.15]+
      beta.elevation * elevation [i, scale.16] + beta.aspect * aspect [i, scale.17] + beta.slope * slope [i, scale.18]+ 
      beta.precipitation * precipitation [i, scale.19] + beta.max_temperature * max_temperature [i, scale.20]+ beta.snow * snow [i, scale.21]+
  ...
  # check observation data for NaN
  if (any(sapply(observation_data, function(x) { any(is.nan(x))}))) {
    NaN_check = sapply(observation_data, function(x) { any(is.nan(x))})
  ...
  inits <-function(){list(
    ...
    scale.1 = sample.int(50,1), # possible scale values
    scale.2 = sample.int(50,1),
    scale.3 = sample.int(50,1),
    scale.4 = sample.int(50,1), 
    scale.5 = sample.int(50,1),
    scale.6 = sample.int(50,1),
    scale.7 = sample.int(50,1),
    scale.8 = sample.int(50,1),
    scale.9 = sample.int(50,1),
    scale.10 = sample.int(50,1),
    scale.11 = sample.int(50,1),
    scale.12 = sample.int(50,1),
    scale.13 = sample.int(50,1),
    scale.14 = sample.int(50,1),
    scale.15 = sample.int(50,1),
    scale.16 = sample.int(50,1),
    scale.17 = sample.int(50,1),
    scale.18 = sample.int(50,1),
    scale.19 = sample.int(50,1),
    scale.20 = sample.int(50,1),
    scale.21 = sample.int(50,1)
  )} 
  timestamp()
  samples <- nimbleMCMC(
    code = Nimble_BLISS,
    ...
    inits = inits,
    monitors = c(
      "beta.0","beta.developed_low_intensity","beta.developed_high_intensity",
